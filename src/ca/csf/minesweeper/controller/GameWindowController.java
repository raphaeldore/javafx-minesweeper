...
public class GameWindowController extends SimpleFXController implements
		Initializable, Observer<GameTile> {
	...
	public void populateGameBoard() {
		ToggleButton[][] gameTiles = new ToggleButton[Configuration.selectedGameDifficulty.nbrOfRows][Configuration.selectedGameDifficulty.nbrOfColumns];
		...
		for (int i = 0; i < Configuration.selectedGameDifficulty.nbrOfRows; i++) {
			for (int j = 0; j < Configuration.selectedGameDifficulty.nbrOfColumns; j++) {
				...
				gameTiles[i][j]
						.setOnMouseReleased(new ToggleButtonEventHandler(i, j));
				gameTiles[i][j].setOnAction(this::disableToggleButtonOnAction);
				gameTiles[i][j].setGraphic(new ImageView(IMAGE_FLAG)); // TODO:
																		// TEMP
	...
	public void openAboutWindow(ActionEvent event) {
		createDialog("../view/AboutWindow.fxml", "À propos",
				new AboutWindowController());
	}

	public void openHelpWindow(ActionEvent event) {
		createDialog("../view/HelpWindow.fxml", "Aide",
				new HelpWindowController());
	}

	private void createDialog(String fxmlPath, String windowName,
			SimpleFXController controller) {
		try {
			SimpleFXScene scene = new SimpleFXScene(controller.getClass()
					.getResource(fxmlPath), controller.getClass().getResource(
					"../view/application.css"), controller);

			SimpleFXStage stage = new SimpleFXStage(windowName,
					StageStyle.UTILITY, scene, this.getSimpleFXApplication(),
